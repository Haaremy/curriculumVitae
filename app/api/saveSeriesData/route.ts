...
export async function POST(request: Request) {
  try {
    ...
    const filePathJsons = path.join(process.cwd(), './public/jsons/series', `${name}.json`);
    ...
    return NextResponse.json({ message: 'Movie data saved successfully' });
  } catch (error) {
    // Handle errors
    console.error('Error saving data:', error);
    return NextResponse.json({ error: 'Failed to save movie data' }, { status: 500 });
  }
}
