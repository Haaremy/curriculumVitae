...
export default function SeriesList({ filenames }: { filenames: string[] }) {
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      setLoading(true);
      setError(null);
      await Promise.all(filteredFilenames.map(name => fetchAndSaveSeriesData(name)));
      setLoading(false);
    };
      ...
      <div className="flex-1 w-full transition-all duration-300">
        <div className="p-4">
          ...
          <table className="min-w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg">
            ...
            <tbody>
              {sortedTeamData.map((team) => team.name!="" && team.punkte>0 && (
                                ...
                                <div className="bg-white dark:bg-gray-700 p-4 rounded-lg shadow mb-2">
                                  ...
                                  {scores.map((score, scoreIndex) => (
                                    <div key={scoreIndex} className="ml-2 mb-2">
                                      ...
                                      <p>
                                        <span>Zeitstempel: {score.stamp}</span>
                                      </p>
                                    </div>
                                  ))}
                                </div>
